...
import {
  ...
  StyledLocateSlider,
} from "../../component_utils/styledElements";
...
const LocateComponent = () => {
  ...
  const updateSelectedMakes = (e) => {
    setSelectedMakes({
      ...selectedMakes,
      [e.target.value]: selectedMakes[e.target.value] ? false : true,
    });
  ...
  const updateSelectedModels = (e) => {
    if (e.target.value in selectedModels) {
      ...
      setSelectedModels({
        ...selectedModels,
        [e.target.value]: selectedModels[e.target.value] ? false : true,
      });
    }
  };
    ...
    <div className="locate-body">
      ...
      <div className="locate-container">
            ...
            <FormControl>
              <Accordion square className="locate-accordion">
                ...
                <StyledAccordionDetails className="locate-accordion-details">
                  <FormGroup>
                    {makes &&
                      Object.entries(makes).map(([makeId, makeObj]) => (
                        <FormControlLabel
                          control={
                            <Checkbox
                              onChange={updateSelectedMakes}
                              value={makeObj.make.id}
                            />
                          }
                          label={makeObj.make.name}
                        />
                      ))}
                  </FormGroup>
                </StyledAccordionDetails>
              ...
              <Accordion className="locate-accordion">
                ...
                <StyledAccordionDetails className="locate-accordion-details">
                  <FormGroup>
                    {selectedMakes &&
                      Object.entries(selectedMakes).map(
                        ([selectedMakeId, truthy]) => {
                          if (truthy)
                            return Object.entries(
                              makes[selectedMakeId].models
                            ).map(([modelId, modelObj]) => (
                              <FormControlLabel
                                control={
                                  <Checkbox
                                    onChange={updateSelectedModels}
                                    value={modelObj.model.id}
                                  />
                                }
                                label={modelObj.model.name}
                              />
                            ));
                        }
                      )}
        ...
        <div className="locate-results-container">
          {locatedDeals &&
            isEmpty(selectedModels) &&
            Object.entries(locatedDeals).map(([dealId, deal]) => (
              <div>
          ...
          {locatedDeals &&
            !isEmpty(selectedModels) &&
            Object.entries(locatedDeals).map(([dealId, deal]) => {
              if (deal.lease_info.trim.model.id in selectedModels)
                return (
                  <div>
                    <DealCard latestDeal={deal} />
                  </div>
